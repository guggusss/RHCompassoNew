<?php
	...
	require_once('db/serverLDAP.php');
		...
		function defineUser($link, $meuNome, $id){
			...
			$nome = $meuNome[0];
			$conta = count($meuNome);
			$sobrenomeOriginal = $meuNome[$conta-1];
		  ...
		  if($verifica > 0 AND $conta == 1){
			$email = $nome."_".$id;
			}

		  $verifica = buscasuporteExiste($link, $email);
		  if($verifica > 0 AND $conta == 2){
			$email = $nome.".".$sobrenome."_".$id;
			}
		  //se tem no banco
		  ...
		  if($verifica2 > 0){
			$sobrenome = $meuNome[$conta-2];
			if($meuNome[$conta-2] == "de" or $meuNome[$conta-2] == "dos" or $meuNome[$conta-2] == "do" or $meuNome[$conta-2] == "das" or $meuNome[$conta-2] == "da"){
			...
			if($verifica3 > 0){
				$sobrenome = $meuNome[$conta-3];
				if($meuNome[$conta-3] == "de" or $meuNome[$conta-3] == "dos" or $meuNome[$conta-3] == "do" or $meuNome[$conta-3] == "das" or $meuNome[$conta-3] == "da"){
			...
			if($verifica4 > 0){
				$sobrenome = $meuNome[$conta-4];
				if($meuNome[$conta-4] == "de" or $meuNome[$conta-4] == "dos" or $meuNome[$conta-4] == "do" or $meuNome[$conta-4] == "das" or $meuNome[$conta-4] == "da"){
			...
			if($verifica5 > 0){
				$sobrenome = $meuNome[$conta-5];
				if($meuNome[$conta-5] == "de" or $meuNome[$conta-5] == "dos" or $meuNome[$conta-5] == "do" or $meuNome[$conta-5] == "das" or $meuNome[$conta-5] == "da"){
			...
			if($verifica6 > 0){
				$sobrenome = $meuNome[$conta-6];
				if($meuNome[$conta-6] == "de" or $meuNome[$conta-6] == "dos" or $meuNome[$conta-6] == "do" or $meuNome[$conta-6] == "das" or $meuNome[$conta-6] == "da"){
					$sobrenome = $meuNome[$conta-3];
				   }
				$email = $nome.".". $sobrenome;
			}

			if($sobrenome == $meuNome[0]){			
				$email = $nome.".".$sobrenomeOriginal;
			}		
	...
	function bancario($conn, $ID_USUARIO, $ENVIO, $RECEBIDO, $PLANILHA_CONTAS, $FORM_COMPR_BANCARIO, $AGENCIA, $NUMERO_CONTA, $TIPO_CONTA, $BANCARIOS_OBS) {
		$query = "UPDATE bancarios set ENVIO = '{$ENVIO}', RECEBIDO ='{$RECEBIDO}', PLANILHA_CONTAS ='{$PLANILHA_CONTAS}', FORM_COMPR_BANCARIO = '{$FORM_COMPR_BANCARIO}', AGENCIA = '{$AGENCIA}', NUMERO_CONTA = '{$NUMERO_CONTA}', TIPO_CONTA = '{$TIPO_CONTA}', BANCARIOS_OBS = '{$BANCARIOS_OBS}' where ID_USUARIO = '{$ID_USUARIO}'";
		return mysqli_query($conn, $query);
	}
	...
	function exame($conn, $ID_USUARIO, $AGENDAMENTO_EXAM_ADM, $ENVIO_FUNC_EXAME, $EMAIL_RECEBIDO_EXAM, $COMENTARIO, $EXAME_OBS){
		$query = "UPDATE exame_admissional set AGENDAMENTO_EXAM_ADM = '{$AGENDAMENTO_EXAM_ADM}', ENVIO_FUNC_EXAME='{$ENVIO_FUNC_EXAME}', EMAIL_RECEBIDO_EXAM = '{$EMAIL_RECEBIDO_EXAM}', COMENTARIO = '{$COMENTARIO}', EXAME_OBS ='{$EXAME_OBS}' where ID_USUARIO = '{$ID_USUARIO}'";
		return mysqli_query($conn, $query);
	}
	...
	function interno ($conn, $ID_USUARIO, $INTRANET_CADASTRO_USUARIO, $INTRANET_CADASTRO_SENHA, $KAIROS_CADASTRO_USUARIO, $KAIROS_CADASTRO_SENHA, $EMAIL_GESTOR_APOIO_SEDE, $EMAIL_INICIO_ATIVIDADES, $EMAIL_BOAS_VINDAS, $ACESSOS, $OBS){
		$query = "UPDATE interno set INTRANET_CADASTRO_USUARIO = '{$INTRANET_CADASTRO_USUARIO}', INTRANET_CADASTRO_SENHA ='{$INTRANET_CADASTRO_SENHA}', KAIROS_CADASTRO_USUARIO = '{$KAIROS_CADASTRO_USUARIO}', KAIROS_CADASTRO_SENHA ='{$KAIROS_CADASTRO_SENHA}', EMAIL_GESTOR_APOIO_SEDE = '{$EMAIL_GESTOR_APOIO_SEDE}', EMAIL_INICIO_ATIVIDADES = '{$EMAIL_INICIO_ATIVIDADES}', EMAIL_BOAS_VINDAS ='{$EMAIL_BOAS_VINDAS}', ACESSOS = '{$ACESSOS}', INTERNO_OBS = '{$OBS}' where ID_USUARIO = '{$ID_USUARIO}'";
		return mysqli_query($conn, $query);
	}
	...
	function recepcao ($conn, $ID_USUARIO, $BOAS_VINDAS_INGR_AGENDADA, $BOAS_VINDAS_INGR_REALIZADA, $BOAS_VINDAS_SALA, $LAYOUT_BOAS_VINDAS_MENSAL, $SURVEY){
		$query = "UPDATE `boas_vindas` SET  `BOAS_VINDAS_INGR_AGENDADA`= '{$BOAS_VINDAS_INGR_AGENDADA}',`BOAS_VINDAS_INGR_REALIZADA`= '{$BOAS_VINDAS_INGR_REALIZADA}',`BOAS_VINDAS_SALA`= '{$BOAS_VINDAS_SALA}', `LAYOUT_BOAS_VINDAS_MENSAL`= '{$LAYOUT_BOAS_VINDAS_MENSAL}', `SURVEY`= '{$SURVEY}' WHERE ID_USUARIO = '{$ID_USUARIO}'";
		return mysqli_query($conn, $query);
	}
